...
from scipy.stats._ksstats import kolmogn, kolmognp, kolmogni
...
class lognorm_gen(rv_continuous):
    ...
    def _fitstart(self, data):  # TODO: delete! pab
        scale = data.std()
        loc = data.min() - 0.001
        logd = log(data - loc)
        m = logd.mean()
...
class truncrayleigh_gen(rv_continuous):  # pab
    ...
    def _ppf(self, q, c):
        return np.sqrt(c * c - 2 * sc.log1p(-q)) - c
